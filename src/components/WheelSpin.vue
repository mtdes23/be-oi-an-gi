...
const lightColors = ['#fff7ed', '#fed7aa', '#ffedd5', '#fde68a']
const darkColors = ['#292524', '#3f3f46', '#27272a', '#352e28']
...
<template>
  <div class="relative mb-8 sm:mb-10">
    ...
    <div class="absolute -top-4 sm:-top-5 left-1/2 -translate-x-1/2 z-20">
      <div class="w-0 h-0 border-l-[14px] sm:border-l-[18px] border-l-transparent border-r-[14px] sm:border-r-[18px] border-r-transparent border-t-[22px] sm:border-t-[26px] border-t-orange-500 drop-shadow-lg filter"></div>
    </div>

    <!-- Wheel ring -->
    <div class="w-56 h-56 sm:w-[340px] sm:h-[340px] md:w-[380px] md:h-[380px] rounded-full p-2 sm:p-3 bg-gradient-to-br from-orange-200 via-amber-100 to-yellow-200 dark:from-orange-900/60 dark:via-amber-900/40 dark:to-yellow-900/50 shadow-xl dark:shadow-orange-900/20"
      ...
      <div class="w-full h-full rounded-full overflow-hidden relative bg-white dark:bg-stone-900 border-4 border-white dark:border-stone-800 card-shadow-lg">
        <svg viewBox="-50 -50 100 100"
          ...
          <path v-for="(item, i) in items" :key="i"
                ...
                stroke="white" stroke-width="0.4" class="dark:stroke-stone-700" />
          <text v-for="(item, i) in items" :key="`text-${i}`"
                ...
                class="pointer-events-none dark:fill-amber-200" style="font-family: 'Be Vietnam Pro', sans-serif;">
            {{ truncate(item?.name || '') }}
          </text>
        </svg>

        <!-- Center -->
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-orange-400 to-amber-400 border-4 border-white dark:border-stone-800 shadow-lg z-20 flex items-center justify-center transition-all duration-300"
             :class="{ 'scale-110 shadow-orange-300/50 dark:shadow-orange-500/30': !isSpinning && hasResult }">
